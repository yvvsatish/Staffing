...
public class Vendordetails extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	@Test
	public void vendorDetails() throws InterruptedException, IOException
	{
		int lastrownum=0;
		f=new FileInputStream(vendordetails);
		...
		com.staffing.vendor.vendordetails vd=PageFactory.initElements(driver,com.staffing.vendor.vendordetails.class);
		m.Vendor();
		m.sm.VendorDetails();
		lastrownum=eo.getRowCount(vendordetails, 0);
		for(int i=1;i<=lastrownum;i++)
		{
			ep.AddNew();
			Row r=ws.getRow(i);
			String title=vd.vendorDetails(driver, r);
			...
			else
			{
				...
				String fail="vendordetailsfail"+Integer.toString(i);
				s.getScreenshot(driver, fail);
				Thread.sleep(2000);
				driver.findElement(By.cssSelector("input.btn.default")).click();
			}
		}
		FileOutputStream f1=new FileOutputStream(vendordetailsres);
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
