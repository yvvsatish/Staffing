...
import com.staffing.editemployee.Editemployeeemergencycontactpage;
import com.staffing.utility.ScreenShot;

public class Editemployeeemergencycontact extends Constants
{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	...
	int count=0;
	@Test
	public void entereditemergencycontactDetails() throws IOException, InterruptedException
	{
		...
		Editemployeeemergencycontactpage eecp=PageFactory.initElements(driver, Editemployeeemergencycontactpage.class);
		l.button();
		Thread.sleep(2000);
				...
				for (int j = 0; j <  editbtn.size(); j++) 
				{
					...
					if(i<= lastrownum)
					{
						for (int k = 1; k < lastrownum;) 
						{
							Thread.sleep(2000);
							editbtn.get(j).click();
							String msgtitle=eecp.editEmployeeEmergencyContactDetails(r);
