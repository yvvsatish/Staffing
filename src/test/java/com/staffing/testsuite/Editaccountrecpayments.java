...
import com.staffing.editaccounts.Editaccountrecpayment;
import com.staffing.utility.ScreenShot;

public class Editaccountrecpayments extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	...
	int count=0;
	@Test
	public void editaccountrecpay() throws IOException, InterruptedException
	{
		...
		Editaccountrecpayment earp=PageFactory.initElements(driver, Editaccountrecpayment.class);
		l.button();
		Thread.sleep(2000);
				...
				for (int j = 0; j <  editbtn.size(); j++) 
				{
					...
					if(i<= lastrownum)
					{
						for (int k = 1; k < lastrownum;) 
						{
							Thread.sleep(2000);
							editbtn.get(j).click();
							String msgtitle=earp.receivedPayments(driver, r);
							...
							else
							{
								...
								String fail="editaccrecpayfail"+Integer.toString(k);
		...
		f1=new FileOutputStream(accountsrecpaymentsres);
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
