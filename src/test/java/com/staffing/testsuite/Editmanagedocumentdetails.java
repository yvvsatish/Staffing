...
public class Editmanagedocumentdetails extends Constants
{
	...
	public void entereditaddressDetails() throws IOException, InterruptedException
	{
		...
		Editmanagedocument emdp=PageFactory.initElements(driver, Editmanagedocument.class);
		l.button();
		Thread.sleep(2000);
				...
				for (int j = 0; j <  editbtn.size(); j++) 
				{
					...
					if(i<= lastrownum)
					{
						for (int k = 1; k < lastrownum;) 
						{
							Thread.sleep(2000);
							editbtn.get(j).click();
							String msgtitle=emdp.editDocumentDetails(driver, r);
