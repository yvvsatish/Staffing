...
import com.staffing.manageemployee.Addmanagecontactpage;
import com.staffing.utility.Autofilloption;
import com.staffing.utility.ScreenShot;
import com.staffing.utility.Slider;

public class Manageemployeecontactdetails extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	@Test
	public  void enterContactDetails() throws IOException, InterruptedException
	{
		String result = null;
		...
		Addmanagecontactpage amap=PageFactory.initElements(driver, Addmanagecontactpage.class);
		...
		for (int i = 1; i <= lastrownum; i++) 
		{
			...
			ep.AddNew();
			Row r=ws.getRow(i);
			String title=amap.AddManageContactdetails(r);
			...
			else
			{
				...
				String fail="managecontactfail"+Integer.toString(i);
				s.getScreenshot(driver, fail);
				Thread.sleep(2000);
				driver.findElement(By.cssSelector("input.btn.default")).click();
			}
		}
		f1=new FileOutputStream(new File(managecontactres));
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
