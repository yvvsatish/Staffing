...
import com.staffing.manageemployee.Addmanageemergencycontactpage;
import com.staffing.utility.Autofilloption;
import com.staffing.utility.ScreenShot;
import com.staffing.utility.Slider;

public class Manageemployeeemergencycontact extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	...
	public  void enterEmergencyContactDetails() throws IOException, InterruptedException
	{
		...
		f=new FileInputStream(manageemergencycontacts);
		...
		Addmanageemergencycontactpage amecp=PageFactory.initElements(driver, Addmanageemergencycontactpage.class);
		...
		driver.findElement(By.id("option")).sendKeys("Employee Emergency Contacts");
		driver.findElement(By.id("submit")).click();
		int lastrownum=0;
		lastrownum=eo.getRowCount(manageemergencycontacts, 0);
		for (int i = 1; i <= lastrownum; i++) 
		{
			Thread.sleep(2000);
			ep.AddNew();
			Row r=ws.getRow(i);
			String title=amecp.addEmployeeContactdetails(r);
			...
			else
			{
				...
				String fail="manageemeergencyfail"+Integer.toString(i);
				s.getScreenshot(driver, fail);
				Thread.sleep(2000);
				driver.findElement(By.xpath("//*[@class='modal fade in']//*[@class='btn default']")).click();
			}
		}
		f1=new FileOutputStream(manageemergencycontactsres);
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
