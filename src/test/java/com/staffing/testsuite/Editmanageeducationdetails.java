...
import com.staffing.editmanageemployee.Editmanageeducationpage;
import com.staffing.utility.Autofilloption;
import com.staffing.utility.ScreenShot;
import com.staffing.utility.Slider;

@Test
public class Editmanageeducationdetails extends Constants
{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	...
	public void entereditaddressDetails() throws IOException, InterruptedException
	{
		...
		Editmanageeducationpage emep=PageFactory.initElements(driver, Editmanageeducationpage.class);
		l.button();
		Thread.sleep(2000);
				...
				for (int j = 0; j <  editbtn.size(); j++) 
				{
					...
					if(i<= lastrownum)
					{
						for (int k = 1; k < lastrownum;) 
						{
							Thread.sleep(2000);
							editbtn.get(j).click();
							String msgtitle=emep.editEducationPage(driver, r);
