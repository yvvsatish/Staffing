...
import com.staffing.salesrecruitment.Profiles;
import com.staffing.utility.ScreenShot;

public class Srprofile extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	@Test
	public  void srProfile() throws IOException, InterruptedException
	{
		String result = null;
		FileOutputStream f1 = null;
		f=new FileInputStream(profileresume);
		...
		Profiles pro=PageFactory.initElements(driver, Profiles.class);
		l.button();
		Thread.sleep(2000);
		...
		Thread.sleep(2000);
		int lastrownum=0;
		lastrownum=eo.getRowCount(profileresume, 0);
		for (int i = 1; i <= lastrownum; i++) 
		{
			Thread.sleep(2000);
			ep.AddNew();
			Row r=ws.getRow(i);
			String title=pro.profile(driver, r);
			...
			else
			{
				...
				String fail="srprofilefail"+Integer.toString(i);
				s.getScreenshot(driver, fail);
				Thread.sleep(2000);
				driver.findElement(By.cssSelector("input.btn.default")).click();
			}
		}
		f1=new FileOutputStream(new File(profileresumeres));
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
