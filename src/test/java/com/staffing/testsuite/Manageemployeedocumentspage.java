...
import com.staffing.manageemployee.Addmanagedocumentspage;
import com.staffing.utility.Autofilloption;
import com.staffing.utility.ScreenShot;
import com.staffing.utility.Slider;

public class Manageemployeedocumentspage extends Constants{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	FileOutputStream f1;
	@Test
	public  void enterDocumentDetails() throws IOException, InterruptedException
	{
		String result = null;
		FileOutputStream f1 = null;
		f=new FileInputStream(managedocuments);
		...
		Addmanagedocumentspage amdp=PageFactory.initElements(driver, Addmanagedocumentspage.class);
		...
		driver.findElement(By.id("option")).sendKeys("Employee Documents");
		driver.findElement(By.id("submit")).click();
		int lastrownum=0;
		lastrownum=eo.getRowCount(managedocuments, 0);
		for (int i = 1; i <= lastrownum; i++) 
		{
			Thread.sleep(2000);
			ep.AddNew();
			Row r=ws.getRow(i);
			String title=amdp.addDocumentDetails(driver, r);
			...
			{
				result=eo.actresultpass(r, 3, 4, 5);
				System.out.println("result:"+result);
				Thread.sleep(2000);
			}
			else
			{
				result=eo.actresultfail(r, 3, 4, 5);
				System.out.println("result:"+result);
				ScreenShot s=PageFactory.initElements(driver, ScreenShot.class);
				String fail="managedocumentfail"+Integer.toString(i);
				s.getScreenshot(driver, fail);
				Thread.sleep(2000);
				driver.findElement(By.cssSelector("input.btn.default")).click();
			}
		}
		f1=new FileOutputStream(managedocumentsres);
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
