...
import com.staffing.editmanageemployee.Editmanagedependentpage;
import com.staffing.utility.Autofilloption;
import com.staffing.utility.ScreenShot;
import com.staffing.utility.Slider;

public class Editmanagedependentdetails  extends Constants
{
	FileInputStream f;
	XSSFWorkbook wb;
	XSSFSheet ws;
	...
	int count=0;
	@Test
	public void editmanagedependentDetails() throws IOException, InterruptedException
	{
		f=new FileInputStream(managedependents);
		...
		Editmanagedependentpage emdp=PageFactory.initElements(driver, Editmanagedependentpage.class);
		l.button();
		Thread.sleep(1000);
		...
		driver.findElement(By.id("option")).sendKeys("Employee Dependants");
		...
		else
		{
			...
			lastrownum=eo.getRowCount(managedependents, 0);
				...
				for (int j = 0; j <  editbtn.size(); j++) 
				{
					...
					if(i<= lastrownum)
					{
						for (int k = 1; k < lastrownum;) 
						{
							Thread.sleep(2000);
							editbtn.get(j).click();
							String msgtitle=emdp.editDependentDetails(r);
							String heading=driver.findElement(By.xpath("//*[@id='btnload']/div/div/div[1]/div[1]")).getText();
							...
							else
							{
								...
								String fail="editmanagedependentfail"+Integer.toString(k);
		...
		f1=new FileOutputStream(managedependentsres);
		wb.write(f1);
		f1.close();
		Thread.sleep(500);
