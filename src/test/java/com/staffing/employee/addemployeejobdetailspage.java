...
public class addemployeejobdetailspage
{
	...
	public String enterjobdetails(WebDriver driver,Row r) throws InterruptedException {
		...
		try{
			jobtype.sendKeys(r.getCell(3).getStringCellValue());
		}catch(Exception e){
		}
		try{
		...
		try{
			locationcode.sendKeys(r.getCell(6).getStringCellValue());
		}catch(Exception e){
		}
		designation.clear();
		try{
		...
		compensation.clear();
		try{
			compensation.sendKeys(r.getCell(11).getStringCellValue());
		}catch(Exception e){
		}
		try{
